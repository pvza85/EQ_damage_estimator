...
def main():
    ...
    args = sys.argv
    if args[1] == 'train':
        train('AWS', int(args[2]), learning_rate = float(args[3]), model_name = model_name)
    else:
        test('AWS', model_name)
...
def train(env, epoch_num, learning_rate = 0.01, clean_start = False, model_name = None):
    ...
    model.fit(X_train, Y_train, validation_set = (X_test, Y_test),n_epoch=epoch_num,  shuffle=True,
              show_metric=True, batch_size=128, snapshot_step=500, snapshot_epoch=False, run_id='inception_ercis_v04')
    #---------------------------------------------------------------------------------------------
